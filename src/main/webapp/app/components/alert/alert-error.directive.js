(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('foodtruckApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-foodtruckApp-error');
                var entityKey = httpResponse.headers('X-foodtruckApp-params');
                if (errorHeader) {
